...
{
	$tip_run_admin=["\xE1A7 SHIFT tuşuna basarak " + this.title + " yöneticisi olarak çalıştır", tip.warning, 1.0]
	$has_admin=key.shift() or key.rbutton()
	
	item(title=title.command_prompt tip=tip_run_admin admin=has_admin image cmd='cmd.exe' args='/K TITLE Komut Satırı &ver& PUSHD "@sel.dir"')
