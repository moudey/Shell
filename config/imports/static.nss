shell
{
	...
	item(mode=mode.multiple
		where=this.id(id.restore_previous_versions,id.cast_to_device,id.give_access_to,id.include_in_library,id.print)
		vis=vis.remove)
	...
	item(find='"open in terminal"|"Open linux shell here"' pos=pos.bottom sep menu='Terminal')
